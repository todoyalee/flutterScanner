                ...
                body: Center(
                  child: ResultScreen(
                      text:
                          ...
                          processRecognizedText(mrzText.toString())
//                            )
                      //)
  ...
  String processRecognizedText(String text) {
    // Define patterns to match specific data types
    RegExp namePattern = RegExp(r'\b[A-Z]+\b');
    RegExp datePattern = RegExp(r'\b\d{2}[-\/.]\d{2}[-\/.]\d{4}\b');
    RegExp nationalityPattern = RegExp(r'\b[A-Z]{3}\b');
    RegExp expirationPattern =
        RegExp(r'SCADENZ.*?(\d{2}[-\/.]\d{2}[-\/.]\d{4})');
    RegExp issuingPattern =
        RegExp(r'(EMISSIONE|ISSUING).*?(\d{2}[-\/.]\d{2}[-\/.]\d{4})');
    RegExp municipalityPattern =
        RegExp(r'(COMUNE DI|MUNICIPALITY).*?([A-Z ]+)');

    // Extract and organize information
    String? municipality = namePattern.firstMatch(text)?.group(0);
    String? givenName =
        namePattern.allMatches(text).skip(6).first.group(0); // Skip the surname
    String? placeAndDateOfBirth = datePattern.firstMatch(text)?.group(0);
    String? nationality = namePattern
        .allMatches(text)
        .skip(31)
        .first
        .group(0); // Skip the surname
    String? sex = namePattern
        .allMatches(text)
        .skip(18)
        .first
        .group(0); // Skip the surname
    String? height = namePattern
        .allMatches(text)
        .skip(12)
        .first
        .group(0); // Skip the surname
    String? expirationDate = expirationPattern.firstMatch(text)?.group(1);
    String? issuingDate = issuingPattern.firstMatch(text)?.group(2);
    String? surname =
        namePattern.allMatches(text).skip(3).first.group(0); // Skip the surname

    // Create a readable format
    String formattedText = '''
Surname: ${surname ?? "N/A"}
Given Name: ${givenName ?? "N/A"}
Place and Date of Birth: ${placeAndDateOfBirth ?? "N/A"}
Nationality: ${nationality ?? "N/A"}
Expiration Date: ${expirationDate ?? "N/A"}
Issuing Date: ${issuingDate ?? "N/A"}
